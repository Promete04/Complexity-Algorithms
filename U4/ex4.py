...
def knapsack(treasure_profit, treasure_weight, max_weight_per_saddles):
    ...
    K = [[0 for _ in range(max_weight_per_saddles + 1)] for _ in range(n + 1)]

    # Build the K array
    for i in range(1, n + 1): # Iterate through each item
        for w in range(max_weight_per_saddles + 1): # Iterate through each weight
    ...
    return K[n][max_weight_per_saddles], K
